...
\problemAnswerDiagram[%
  ...
  answer text one = {\emph{Correto.} Redes já são um tópico bastante complexo para quem está começando. Com o movimento 1, Preto aprisiona a pedra branca, não há escapatória, e nem como ganhar mais liberdades. Em japonês, o termo para rede é \emph{\gls{geta}}; em inglês, \emph{net}.},
  answer text two = {\emph{Incorreto.} Preto 1 é uma captura em escada, o que depende do resto do tabuleiro. É preferível algo mais incondicional, como uma rede, que não depende de quebra-escadas.},
]
...
\problemAnswerDiagram[%
  ...
  problem text    = {Parece ser uma rede mais complexa, mas pertence à mesma família dos problemas anteriores.},
  answer text one = {\emph{Correto.} O \emph{tesuji}, isto é, o movimento brilhante, será muito parecido com os anteriores.},
...
\problemAnswerDiagram[%
  ...
  answer text two = {\emph{Incorreto.} Branco pode escapar. Não é um desastre tão grande pois escapar com 2 leva a um ``\gls{squeeze}'' ou ``espremida''. No caso, Preto sacrifica suas pedras para obter uma força bastante considerável no exterior. Escapar com A gera uma espremida pior para Branco, após Preto B.},
]
...
\problemAnswerDiagram[%
  ...
  answer text two = {\emph{Incorreto.} Preto 1 é um movimento simples e, em geral, é boa técnica atravessar o \emph{\gls{keima}} --- movimento do cavaleiro, ou o ``L'' do cavalo no xadrez --- de A para B do adversário, mas, desta vez, não funciona.},
]

\problemAnswerDiagram[%
  sgf folder      = liberdades/redes,
  sgf filename    = redes.2,
  problem text    = {Capturar pedras de corte é frequentemente de suma importância no Go.},
...
\problemAnswerDiagram[%
  ...
  answer text one = {\emph{Correto.} Preto conecta seus grupos e se torna muito mais forte. Branco não deveria tentar escapar, se não há como fazê-lo.},
  answer text two = {\emph{Incorreto.} A rede de Preto 3 parece funcionar, mas Preto não possui muitas liberdades com o grupo A.},
]
...
\problemAnswerDiagram[%
  ...
  answer text two = {\emph{Incorreto.} Em 99\% dos casos, capturar em escada, ou seja, uma captura condicional, não é ideal, claro, se uma captura incondicional for possível.},
]
...
\problemAnswerDiagram[%
  ...
  problem text    = {Este é um dos poucos \emph{\glspl{tesuji}} --- movimento brilhante --- que possui nome próprio.},
  answer text one = {\emph{Correto.} Em inglês, o nome desta rede é ``\gls{crane's nest}'', ou ``ninho do grou'', expressão que vem da China. Um dos principais aspectos para que esta rede funcione é ter as pedras A e B.},
...
\problemAnswerDiagram[%
  ...
  answer text two          = {\emph{Incorreto.} 1, 3 e 5 parecem funcionar por colocar maior pressão, mas Preto termina com muitas fraquezas, a pedra marcada fazendo a diferença.},
