...
\problemAnswerDiagram[%
  ...
  answer text one = {\emph{Correto.} Este padrão é conhecido como \emph{shicho} em japonês, ou ``escada'' em português. Ao chegar na borda do tabuleiro, Branco não conseguirá mais estender suas liberdades. Tanto A quanto B funcionarão para completar a captura quando chegamos a 1 linha da lateral.},
...
\problemAnswerDiagram[%
  ...
  answer text two = {\emph{Incorreto.} A escada ainda não funciona. A pedra branca A colocará a pedra B em atari inevitavelmente.},
]
...
\problemAnswerDiagram[%
  ...
  answer text one = {\emph{Correto.} A escada não funciona desta vez. O melhor que Preto pode fazer é um misto de compromisso com dano maior contra o quebra-escada branco no exterior. Como diria Toshiro Kageyama 7p, não há atalhos, é preciso ler a escada!},
...
\problemAnswerDiagram[%
  ...
  problem text    = {Este é um \emph{joseki} --- isto é, uma sequência ótima para os dois lados. Preto possui basicamente 2 opções a seguir.},
...
\problemAnswerDiagram[%
  ...
  answer text one          = {\emph{Correto.} Ao jogar em 1 e estender em 3, Preto cria  \emph{miai} --- isto é, duas opções equivalentes --- de capturar em escada com A, ou capturar as pedras de corte no topo com B.},
...
\problemAnswerDiagram[%
  ...
  answer text two = {\emph{Incorreto.} Começar pelo atari de 1 parece gerar a mesma sequência, porém de maneira mais simples. Mas Branco terá o tempo de capturar as pedras pretas mais críticas!},
]
