...
\problemAnswerDiagram
  ...
  {\emph{Correto.} Branco não deveria jogar \textLmr{2}, pois não há para onde fugir. Suas pedras terão sempre \textLmr{1} liberdade, mas vão colidir com o canto esquerdo no final.}
...
\problemAnswerDiagram
  ...
  {\emph{Variação.} Ao escapar com esta pedra, Branco vai de \textLmr{1} liberdade para \textLmr{3}, o que é bastante eficiente, isto é, \textLmr{2} liberdades por movimento; em lutas, liberdades são um bem crucial. Desta maneira, Branco também expõe os cortes A e B no exterior preto.}
...
\problemAnswerDiagram
  ...
  {\emph{Correto.} Note que não é suicídio jogar em \textLmr{1}, pois a regra da captura possui precedência. Isto é, primeiro aplicamos a regra da captura, se possível, e, só depois, examinamos se é suicídio. E segue que, se algo for capturado, haverá mais de uma liberdade.}
  {\emph{Variação.} Mais tarde, se Branco conseguir pedras no exterior, quem pode ser capturado é o Preto! Antes de \textLmr{1}, Preto poderia finalmente capturar o canto, e, dessa maneira, evitar de ser capturado.}

\problemAnswerDiagram
  {captura}
  {captura.6}
  {É possível capturar algo branco? Ou é suicídio?}
  {\emph{Correto.} Sim, é possível capturar duas pedras brancas. Em seguida, a pedra preta que efetua a captura estará imediatamente em \emph{atari}, isto é, ela possuirá somente \textLmr{1} liberdade, então Branco terá a opção de uma contra- ou recaptura.}
...
\problemAnswerDiagram
  {captura}
  {captura.7}
  {O grupo branco mais ao topo possui \textLmr{2} pedras. Isso muda algo em relação à possibilidade de captura?}
  {\emph{Correto.} Apesar de o grupo branco ter mais pedras, ele ainda possui somente \textLmr{1} liberdade. Esta forma é conhecida como ``casco de tartaruga'', que é o número mínimo de pedras necessário para se capturar \textLmr{2} pedras. (Note que ambas as pedras brancas serão retiradas do tabuleiro.)}
